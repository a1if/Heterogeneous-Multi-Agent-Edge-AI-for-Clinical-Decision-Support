...
class PerceptionAgent:
    ...
    def predict(self, raw_segment: np.ndarray, rr_interval_ms: float | None = None,
        ...
        if sqi < SQI_LOW_THRESHOLD:
            ...
            # classification.confidence is the TRUE softmax probability, NOT the
            # renormalized one. These deliberately differ, and that asymmetry is the
            # point: `top_3` is renormalized only to satisfy the schema's
            # top_3_sums_to_one validator (a presentation constraint), whereas
            # `confidence` is the clinical decision variable — it is what the
            # escalation rule below tests against 0.85, and what OUTPUT_INSTRUCTIONS
            # hands the Reasoning Agent as the same 0.85 criterion.
            # Renormalizing divides by a sum < 1.0, so it can only inflate: measured
            # on DS2 at mean +0.044 / max +0.073, which pushed 30 of 2,582
            # predicted-V/F beats (1.2%) over the 0.85 line and set
            # requires_urgent_review on beats the model was NOT that confident about.
            # Do not "tidy" this back to renorm_top3[0] — that reintroduces the bug.
            # (Verified when fixing: none of those 30 fall in the 80-event Day 6/7
            # evaluation set, so the reported headline figures are unchanged.)
            confidence = float(raw_top3[0])
            top_3 = [
...
def replay_selected(agent: "PerceptionAgent", X, rr, record_ids, selected):
    ...
    Therefore:
      ...
      - within a needed record, beats after its last selected index can be skipped.
    Both are exactly what the naive `for i in range(max(selected) + 1)` loop
    spends most of its time on. On the DS2 80-event selection this cuts the
    replay from 44,341 predict() calls to 15,964 (-64%) with byte-identical
    output, verified by direct comparison against the naive loop.

    Requires records to be contiguous runs in record_ids -- the same assumption
    the naive loop already makes (it resets whenever record_id changes, so a
    revisited record id would already have reset its own state mid-record).
    """
    selected = list(selected)
    record_ids = np.asarray(record_ids)

    # Last selected index per record, and each record's first index (contiguous runs).
    last_needed = {}
    for i in selected:
        rec = int(record_ids[i])
        if i > last_needed.get(rec, -1):
            last_needed[rec] = i

    boundaries = np.flatnonzero(np.diff(record_ids) != 0) + 1
    starts = np.concatenate(([0], boundaries))
    record_start = {int(record_ids[s]): int(s) for s in starts}
