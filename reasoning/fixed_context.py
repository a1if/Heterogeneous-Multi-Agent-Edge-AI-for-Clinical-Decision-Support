...
FIXED_CONTEXT = {
    ...
    "V": (
        ...
        "structural heart disease is usually benign. Two separate, independent "
        "criteria each raise concern for a more serious ventricular arrhythmia "
        "and warrant prompt clinical review: three or more consecutive "
        "occurrences in a row, OR a single occurrence classified with high "
        "confidence (greater than 0.85), particularly when accompanied by a "
        "wide QRS duration. Either criterion alone is sufficient to warrant review."
    ),
    "F": (
        ...
        "The same two independent criteria as ventricular ectopy warrant prompt "
        "clinical review: three or more consecutive abnormal beats in a row, "
        "OR a single occurrence classified with high confidence (greater than 0.85)."
    ),
    "Q": (
